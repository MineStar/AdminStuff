...
public class cmdBlockCount extends AbstractCommand {
	...
	public cmdBlockCount(String syntax, String arguments, String node,
	...
	public void execute(String[] args, Player player) {
		...
		if (pManager.isInSelectionMode(player)) {
			...
			if (corner == null || corner[0] == null || corner[1] == null) {
				PlayerUtils.sendError(player, pluginName,
						"Du hast keine zwei Bloecke selektiert!");
				return;
			}
		...
		// activate selection mode
			...
			PlayerUtils.sendSuccess(player, pluginName,
					"Du bist nun im Selektionsmodus!");
			PlayerUtils
					.sendInfo(player, pluginName,
							"Markiere mit einem Linksklick Block 1 und mit Rechtsklick Block 2!");
			PlayerUtils.sendInfo(player, pluginName,
					"Fuehre dann den Befehl noch einmal aus!");
		}
	}
	...
	private void printStatistic(Player player,
			Map<Material, Counter> blockCounter, Counter total) {
		PlayerUtils.sendSuccess(player, pluginName,
				"Es befinden sich " + total.getCount()
						+ " Bloecke in dem Gebiet!");
		Counter counter = null;

		for (Entry<Material, Counter> entry : blockCounter.entrySet()) {
			counter = entry.getValue();
			if (counter.getCount() != 0L) {
				double percent = ((double) counter.getCount() / (double) total
						.getCount()) * 100.0;
				percent = Math.round(percent * 100.0) / 100.0;
				PlayerUtils.sendInfo(player, entry.getKey().name() + " : "
						+ counter.getCount() + " of " + total.getCount()
						+ " ( " + percent + "% )");
			}
		}
	...
	private Vector getMaximum(Location[] corner) {
		// @formatter:off
		return new Vector(Math.max(corner[0].getX(), corner[1].getX()),
				Math.max(corner[0].getY(), corner[1].getY()), Math.max(
						corner[0].getZ(), corner[1].getZ()));
		// @formatter:on
	}

	private Vector getMinimum(Location[] corner) {
		// @formatter:off
		return new Vector(Math.min(corner[0].getX(), corner[1].getX()),
				Math.min(corner[0].getY(), corner[1].getY()), Math.min(
						corner[0].getZ(), corner[1].getZ()));
		// @formatter:on
	}
