...
public class cmdStack extends AbstractCommand {
		...
		if (args.length == 0)
			ConsoleUtils
					.printError(pluginName,
							"You don't even have an inventory, so how can you stack it?");
		else
	...
	private void stack(Player target, CommandSender sender) {
			...
			if (item.getAmount() < max) {
				...
				for (int j = i + 1; j < len; j++) {
					...
					if (item2.getTypeId() == item.getTypeId()
							&& ((!ItemType.usesDamageValue(item.getTypeId()) && ignoreDamaged) || item
									.getDurability() == item2.getDurability())
							&& ((item.getItemMeta() == null && item2
									.getItemMeta() == null) || (item
									.getItemMeta() != null && item
									.getItemMeta().equals(item2.getItemMeta())))) {
		...
		if (!sender.getName().equalsIgnoreCase(target.getName())) {
			ChatUtils.writeSuccess(sender, pluginName, "Items des Spielers '"
					+ target.getName() + "' wurden gestackt!");
		}
	}
}
