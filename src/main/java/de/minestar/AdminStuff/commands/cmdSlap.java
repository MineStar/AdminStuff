...
public class cmdSlap extends AbstractCommand {
	...
	private void slap(String targetName, CommandSender sender) {
		...
		if (target == null)
			...
			Boolean wasSlapped = mPlayer.getBoolean("adminstuff.slapped");
			if (wasSlapped == null || !wasSlapped)
				wasSlapped = true;
			else
				wasSlapped = false;
			mPlayer.setBoolean("adminstuff.slapped", wasSlapped);

			if (wasSlapped) {
				Bukkit.broadcastMessage(ChatColor.BLUE + sender.getName()
						+ " schlaegt " + mPlayer.getNickName()
						+ " mit einem kalten Fisch ins Gesicht!");
				pManager.updatePrefix(mPlayer);
			} else {
				pManager.updatePrefix(mPlayer);
				Bukkit.broadcastMessage(ChatColor.BLUE + mPlayer.getNickName()
						+ "s Fisch wurde von einer Sturmmoeve gefressen!");
			}
		}
	}
}
